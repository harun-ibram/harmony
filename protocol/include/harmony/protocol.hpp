...
namespace harmony::proto
{
  ...
  */
  ...
  inline void parseLogin(std::string &json, Message &msg)
  {
    // Find the username field and extract username
    std::string_view from_key = "\"from\":\"";
    size_t from_pos = json.find(from_key);
    if (from_pos == std::string_view::npos)
    {
      msg.type = harmony::proto::Type::Error;
      return;
    }

    from_pos += from_key.size();
    size_t from_end = json.find("\"", from_pos);
    std::string user = json.substr(from_pos, from_end);
    msg.from = user;
  }

  inline void parseChat(std::string &json, Message &msg)
  {
    // Find the username field and extract username
    std::string_view from_key = "\"from\":\"";
    size_t from_pos = json.find(from_key);
    ...
    // Find chat text and extract it
    std::string_view body_key = "\"body\":{";
    size_t body_pos = json.find(body_key, from_end);
    ...
    body_pos += body_key.size();
  
    std::string_view text_key = "\"text\":\"";
    size_t text_pos = json.find(text_key, body_pos);
    ...
    text_pos += text_key.size();
    size_t text_end = json.find("\"", text_pos);
    ...
    std::string text = json.substr(text_pos, text_end);
    msg.text = text;
  }
